...
function _mt:run_api_migrations()
  local conn = self:get_stored_connection()
  if not conn then
    error("no connection")
  end
  ...
  for plugin, err in self:iterate([[
    ...
    if api_id and api_id ~= null then
      ...
      plugins[api_id][plugins[api_id].n] = plugin.id
    end
  end
  ...
  for i = 1, apis.n do
    ...
    local route_id = utils.uuid() --api.id
...
%s
  DELETE FROM apis
        WHERE id = %s;
COMMIT;]],
      escape(service.name or route.id),
      escape(service.id),
